...
**State** (2026-09-25): **v0.1.0 released** — notarized DMG from `scripts/release.sh` (Developer ID, widget extension included, fanart.tv project key injected at build time from `.env`, never committed; `LayeredSecretStore` falls back to it). Website live at https://msitarzewski.github.io/AudioPaper/ (Pages via Actions). Security hardening shipped in 4329c39. Next: screenshots for the site — user asked whether to automate with pop/K-pop; pending their choice on what may appear (recommendation: album covers + Wikimedia Commons photos with credits visible; no fanart.tv/DeviantArt fan art; capture only AudioPaper windows or the rendered wallpaper file, never the screen).
