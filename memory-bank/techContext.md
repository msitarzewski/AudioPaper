...
- **Icon**: shipped design is `display` in red — GlassPowerTools' display (identical bezel/screen geometry) with the Heroicons solid musical note on the screen (MIT, `THIRD-PARTY-NOTICES.md`) and a stand. An earlier `camera` design is kept for reference. Masters are white SVG masks in `docs/icon/Sources/<design>/`; `python3 docs/icon/build_icons.py [--ship <design> <palette>]` builds every design × palette (red, cobalt, emerald, violet, amber) with vector linear-gradient fills per appearance, renders every appearance with Icon Composer's `ictool` (`Xcode-beta.app/Contents/Applications/Icon Composer.app/Contents/Executables/ictool`), and writes `palettes/board-<design>.png` + `board-compare.png`. Icon Composer allows at most 4 groups (a group may hold several layers), maps an SVG viewBox onto the icon grid (layers carry a 1.22 scale; display ×1.12 more), and honours a linear-gradient `orientation` (`{"start":{"x":0,"y":0},"stop":{"x":1,"y":1}}`) and alpha in `srgb:r,g,b,a`. The display matches GlassPowerTools' look with an inset-rim shadow layer (no glass), a diagonal vivid screen and silver bezel in both appearances. Menu bar icons are template SVGs of the same display (18.4×16 pt, viewBox cropped; the note path is baked into the screen path at menu bar scale, knocked out with even-odd). See `docs/icon/README.md`.
- **External services**
