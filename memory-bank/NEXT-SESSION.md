...
## Next
1. **Publish 0.1.1** if not yet done: the DMG is built and notarized (`build/release/AudioPaper-0.1.1.dmg`); commit, push, tag `v0.1.1` and `gh release create` with the notes and checksum — only with the user's go-ahead. Future releases: bump `project.yml`, `set -a; source ~/.config/brew-browser/signing.env; set +a; scripts/release.sh`.
