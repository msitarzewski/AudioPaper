...
### 2026-09-25: Check for Updates
- Manual only (no background check, Apple frameworks only; Sparkle deferred): `UpdateChecker` reads `tag_name` from `api.github.com/repos/…/releases/latest` through the hardened client; `UpdatePrompt` shows an NSAlert with **Download…**, which opens the fixed releases page (never a URL from the reply). In the menu bar menu and the app menu (after About). NETWORK/PRIVACY/README/Help/Reference/Shortcuts updated. Verified live ("up to date" on 0.1.2). Also: CI timing fix for `rapidSkipsLoadOnlyTheTrackThatSettles` (wait for the desktop, not `showing`).

### 2026-09-25: Sparkle auto-update and the internet-content disclaimer (0.1.3)
- Mirrors Anomalous (Sparkle 2.9.4) but sandboxed and without a server: feed on GitHub Pages, enclosures on GitHub Releases, own EdDSA key (`--account AudioPaper`). Replaced the manual GitHub-API checker built earlier the same day. A local end-to-end test caught the wrong Info.plist key (`SUEnableInstallerLauncher` → `SUEnableInstallerLauncherService`); after the fix 0.1.3-test updated itself to 0.1.4-test. Also fixed a pipefail/grep -q false failure in release.sh's Sparkle signature check. Disclaimer: Settings → Sources footer, About, README, Help, landing. Sparkle's licence in THIRD-PARTY-NOTICES; CONTRIBUTING records it as the one allowed dependency.
